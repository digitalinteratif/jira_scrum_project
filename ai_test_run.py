...
def dashboard():
    ...
    for l in links:
        ...
        rows += f"<tr><td>{l.id}</td><td>{l.title or ''}</td><td><a href='{short_url}' target='_blank'>{short_url}</a></td><td>{l.created_at}</td><td><a href='{url_for('link_stats', link_id=l.id)}'>Stats</a> | <a href='{url_for('edit_link', link_id=l.id)}'>Edit</a> | <form style='display:inline' method='POST' action='{url_for('delete_link', link_id=l.id)}'><input type='hidden' name='csrf_token' value='{{{{ csrf_token() }}}}'><button type='submit'>Delete</button></form></td></tr>"
    content = f"""
...
#   * No Node.js or JS backend components are introduced.
#
# -----------------------------------------------------------------------------
