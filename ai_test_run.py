...
class User(Base):
    ...
    # New: account activation flag. New users are inactive until they verify their email.
...
def create_user(email: str, password: str, session=None) -> User:
    ...
    try:
        ...
        # New users created inactive by default; registration flow will issue verification token.
        user = User(email=email, password_hash=pwd_hash, is_active=False, created_at=datetime.utcnow())
        session.add(user)
        session.commit()
...
def register_post():
    ...
    try:
        sess = get_session()
        try:
            # Use create_user helper (which uses hashing if available) and creates user with is_active=False
            user = create_user(email=email, password=password, session=sess)

            # Generate an email verification token for the new user (purpose='email_verify')
            try:
                jti = str(uuid4())
                # 24 hours expiry (configurable via EMAIL_VERIFICATION_EXP_SECONDS)
                verify_exp = int(current_app.config.get("EMAIL_VERIFICATION_EXP_SECONDS", 24 * 3600))
                token = jwt_utils.create_access_token(user_id=user.id, jti=jti, purpose="email_verify", exp_seconds=verify_exp)
                verify_url = url_for("auth.verify_email_get", token=token, _external=True)
                # Dev stub: log the verification link so tests/dev can extract it
                current_app.logger.info("Email verification link (dev): %s", verify_url)
                _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z ISSUED_VERIFY email={email} user_id={user.id} jti={jti} verify_url={verify_url}")
                # Note: We do not set user.is_active=True here. Activation occurs when link is visited.
            except Exception:
                current_app.logger.exception("Error issuing email verification token for user_id=%s", getattr(user, "id", None))
                _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z ISSUED_VERIFY_FAILED email={email} user_id={getattr(user, 'id', None)}")

            # Return success page which instructs user to check email (dev: logged link)
            return render_layout(_verify_sent_template, email=user.email)
...
# -------------------------
# KAN-110: Email verification helpers, templates and endpoints (surgical additions to auth blueprint)
# -------------------------

# Templates for verification pages (must include consistent layout via render_layout)
_verify_sent_template = """<h2>Registration Successful — Verify Your Email</h2>
<p class="success">An email verification link has been generated (dev: logged). Please click the link in that message to activate your account.</p>
<p>If you did not receive an email, you may request a new verification link from your account page.</p>
<p><a href="/auth/">Back to Auth Index</a></p>
"""

_verify_success_template = """<h2 class="success">Email Verified</h2>
<p class="success">Thank you! Your email address has been verified and your account is now active.</p>
<p><a href="/auth/login">Sign in</a></p>
"""

_verify_error_template = """<h2 class="error">Email Verification Error</h2>
<p class="error">{{ message }}</p>
<p><a href="/auth/">Back to Auth Index</a></p>
"""

# Trace appender for KAN-110 (lightweight; append-only)
def _append_trace_kan110(entry: str):
    try:
        trace_path = os.path.join(os.getcwd(), "trace_KAN-110.txt")
        with open(trace_path, "a", encoding="utf-8") as fh:
            fh.write(entry.rstrip() + "\n")
    except Exception:
        current_app.logger.exception("Failed to write KAN-110 trace file")
...
def verify_email_get(token):
    """
    Verify the email address using a purpose-scoped token.

    Flow:
      - Decode token using jwt_utils.decode_token (enforces signature and expiry).
      - Ensure payload.purpose == 'email_verify'.
      - Ensure required claims (user_id, jti) present.
      - Check jwt_utils.is_revoked(jti) to avoid accepting revoked tokens.
      - Activate the user (set is_active=True) and commit.
      - Optionally insert a RevokedToken record for the token's jti (to prevent reuse).
      - Render success or helpful error messages via render_layout.

    Errors:
      - ExpiredSignatureError -> friendly expiry message
      - InvalidTokenError / other decode errors -> friendly invalid message
      - Wrong purpose -> invalid
      - DB errors -> internal error message (safe)
    """
    try:
        payload = jwt_utils.decode_token(token)
    except ExpiredSignatureError:
        _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_FAILED_EXPIRED token={token[:32]}...")
        return render_layout(_verify_error_template, message="Verification link has expired"), 400
    except InvalidTokenError:
        _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_FAILED_INVALID token={token[:32]}...")
        return render_layout(_verify_error_template, message="Invalid verification link"), 400
    except Exception:
        current_app.logger.exception("Unexpected error decoding verification token")
        _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_FAILED_DECODE token={token[:32]}...")
        return render_layout(_verify_error_template, message="Invalid verification link"), 400

    # Validate purpose
    purpose = payload.get("purpose")
    if purpose != "email_verify":
        _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_FAILED_WRONG_PURPOSE payload_purpose={purpose}")
        return render_layout(_verify_error_template, message="Invalid verification link"), 400
    ...
    exp_ts = payload.get("exp")
    if not jti or not user_id:
        _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_FAILED_MISSING_CLAIMS payload={payload}")
        return render_layout(_verify_error_template, message="Invalid verification link"), 400

    # Check revocation
    try:
        if jwt_utils.is_revoked(jti):
            _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_FAILED_REVOKED jti={jti} user_id={user_id}")
            return render_layout(_verify_error_template, message="This verification link is no longer valid"), 400
    except Exception:
        current_app.logger.exception("Error checking revocation for jti=%s", jti)
        _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_FAILED_REVOCATION_CHECK_ERROR jti={jti}")
        return render_layout(_verify_error_template, message="Unable to validate verification link"), 500

    # Activate user and insert RevokedToken to prevent reuse
    sess = None
    try:
        sess = get_session()
        user = sess.query(User).filter(User.id == int(user_id)).first()
        if not user:
            _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_FAILED_NO_USER user_id={user_id}")
            return render_layout(_verify_error_template, message="Invalid verification link"), 400

        if user.is_active:
            # Idempotent success: already active
            _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_ALREADY_ACTIVE user_id={user_id} jti={jti}")
            return render_layout(_verify_success_template)
        ...
        try:
            ...
            # Compute expires_at if available
            expires_at = None
            try:
                if exp_ts is not None:
                    expires_at = datetime.fromtimestamp(int(exp_ts), tz=timezone.utc)
            ...
            # Insert RevokedToken record for the verification token jti to prevent replay
            rt = RevokedToken(jti=jti, revoked_at=datetime.now(tz=timezone.utc), expires_at=expires_at)
            sess.add(rt)
            ...
            try:
                ...
                # If the RevokedToken already exists, rollback and try to persist user activation separately
                sess.rollback()
                try:
                    sess.add(user)
                    sess.commit()
                    ...
                    sess.rollback()
                    current_app.logger.exception("Failed to commit activation after revocation conflict for user_id=%s", user_id)
                    return render_layout(_verify_error_template, message="Internal error"), 500
        except Exception:
            sess.rollback()
            current_app.logger.exception("Error activating account for user_id=%s", user_id)
            return render_layout(_verify_error_template, message="Internal error"), 500

        _append_trace_kan110(f"{datetime.utcnow().isoformat()}Z VERIFY_SUCCESS user_id={user.id} jti={jti}")
        return render_layout(_verify_success_template)
    finally:
        if sess:
            try:
