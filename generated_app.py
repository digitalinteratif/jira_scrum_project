...
def render_with_layout(content_template, **context):
    ...
    Defensive against TemplateNotFound when templates attempt to extend missing files.
    """
    ...
    try:
        ...
        # Defensive: present a friendly error UI rather than raising a TemplateNotFound
...
def get_session(session_id: str):
    ...
    if not row:
        return None
    # SQLite may return timestamps as strings; normalize
...
def index():
    ...
    # Render the index HTML inside the single-file LAYOUT wrapper.
...
def register():
    ...
    # Note: find_user_by_username_or_email accepts a single identifier; first check username collisions
    ...
    # Check specifically email to avoid false positives
    ...
    # After registration, do not auto-login; require verification.
...
def dashboard_shorten():
    ...
    # create_url enforces owner_id at insert time
...
def dashboard_manage_url(url_id):
    ...
    conn = get_db()
    cur = conn.cursor()
    # Enforce owner filtering: strictly by owner_id = current_user.id
...
def redirect_code(code):
    ...
    # privacy check (ensure owner-only access for private links)
    # Note: is_private may be stored as 0/1; treat truthily
...
def password_reset_request():
    ...
    if user:
        ...
        # Create the password reset tied strictly to the user_id
...
def password_reset(token):
    ...
    # update password - the reset entry contains the correct user_id to act upon
...
if __name__ == "__main__":
    # Basic template sanity check: ensure no templates use external 'extends' which is unsupported in this single-file app.
    problematic = []
